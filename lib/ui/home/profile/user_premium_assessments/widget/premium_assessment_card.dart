...
class PremiumAssessmentCard extends StatefulWidget {
  const PremiumAssessmentCard({Key? key, required this.assessmentDto}) : super(key: key);
...
class _PremiumAssessmentCardState extends State<PremiumAssessmentCard> {
  ...
  Widget build(BuildContext context) {
    ...
    PremiumAssessmentCardController controller = Get.put(PremiumAssessmentCardController());
    return InkWell(
      onTap: () async {
        Get.to(
              () => AssessmentDetailPage(assessmentDto: widget.assessmentDto)
        );
      },
      child: Card(
      elevation: 1,
